...
# ensure_ini_value <section> <key> <value>
# Guarantees [section]\nkey=value exists in $WSL_CONF.
ensure_ini_value() {
  local section="$1" key="$2" value="$3"
  local full_line="${key}=${value}"

  # If the exact key=value already present, nothing to do
  if grep -qF "$full_line" "$WSL_CONF"; then
    return
  fi

  # If the key exists with a DIFFERENT value, replace it in-place
  if grep -qE "^\s*${key}\s*=" "$WSL_CONF"; then
    sed -i "s|^\s*${key}\s*=.*|${full_line}|" "$WSL_CONF"
    changed=true
    return
  fi

  # Key doesn't exist — ensure section header exists, then append key under it
  if ! grep -qF "[$section]" "$WSL_CONF"; then
    printf '\n[%s]\n' "$section" >> "$WSL_CONF"
  fi
  sed -i "/^\[${section}\]/a ${full_line}" "$WSL_CONF"
  changed=true
}

ensure_ini_value "boot" "systemd" "true"
ensure_ini_value "boot" "command" "\"mount -t binfmt_misc binfmt_misc /proc/sys/fs/binfmt_misc; echo ':WSLInterop:M::MZ::/init:PF' > /proc/sys/fs/binfmt_misc/register\""
ensure_ini_value "network" "generateResolvConf" "false"
...
$wslConfResult = wsl -d Ubuntu -u root -- bash -c $ensureWslConfScript
if ($LASTEXITCODE -ne 0) {
    Write-Host "codespace-ubuntu-wsl script exited with an error: $LASTEXITCODE"
    exit $LASTEXITCODE
}
$wslConfChanged = $wslConfResult -match "CHANGED"

if ($wslConfChanged) {
  Write-Host "wsl.conf has been updated."

  # Restart WSL to apply wsl.conf changes
  Write-Host "Shutting down WSL to apply wsl.conf changes and waiting 3 seconds..."
  wsl --shutdown
  Start-Sleep -Seconds 3
  Write-Host "Restarting WSL Ubuntu..."
  wsl -d Ubuntu -u root -- bash -c "echo 'Ubuntu restarted successfully'"
  ...
  Write-Host "wsl.conf already has all required settings."
}

# Ensure resolv.conf has the correct nameserver
$grepResolvConfOutput = wsl -d Ubuntu -u root -- bash -c "grep -q 'nameserver 8.8.8.8' /etc/resolv.conf && echo 'OK'"
if ($grepResolvConfOutput -match "OK") {
  Write-Host "resolv.conf is already correctly configured."
} else {
  Write-Host "Fixing resolv.conf in Ubuntu..."
  wsl -d Ubuntu -u root -- bash -c "chattr -f -i /etc/resolv.conf || true"
  wsl -d Ubuntu -u root -- bash -c "rm -f /etc/resolv.conf"
  wsl -d Ubuntu -u root -- bash -c "echo 'nameserver 8.8.8.8' > /etc/resolv.conf"
  wsl -d Ubuntu -u root -- bash -c "chattr -f +i /etc/resolv.conf"
