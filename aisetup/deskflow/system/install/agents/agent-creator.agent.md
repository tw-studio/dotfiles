...
- Do NOT publish to `.claude/skills/` without user's explicit go-ahead. `02-proposals/` and `03-drafts/` are where work lives; `.claude/skills/` is where only shipped commands live. Commissioned runs never publish — the commissioning agent takes the finished file from the project folder and delegates to it.
...
- Do NOT create or modify files outside the command's project folder, `.claude/skills/`, and `.github/agents/` (publishing only).
...
2. **Read the workspace.** Existing fleet in `.claude/skills/` (collisions, house style, trigger overlap), AGENTS.md, workspace instructions, and — on improve and retrospect runs — the command's project folder, its session records, and prior versions.
...
1. **Publish on go-ahead.** Copy the draft to `.claude/skills/{name}/SKILL.md`, commit and push. Every published version gets locked in by a commit — that's what makes fearless iteration possible.
2. **Also a custom agent, whenever user says so.** At any point — most often at plan approval — user may say a command should also be a custom agent file. Never ask; just know how: publish a twin copy to `.github/agents/{name}.agent.md`, identical in every way but one — its frontmatter `name` is the title-cased version of the kebab slug (`agent-creator` becomes `Agent Creator`). Keep both copies in lockstep through every republish, because twins updated separately stop being twins after one revision.
...
**Orchestration** — coordinating several concurrent workers is its own published command in `.claude/skills/`, referencing the worker files it delegates to. Any single spec should work both invoked by name in chat and handed to a spawned instance.
...
- **Scannable structure.** Short paragraphs under bold inline headings that stand alone in meaning — roughly one longer sentence or 2–3 short ones per block, two longer sentences when warranted. Lists with per-item headings, tasteful bolding, and tables are all welcome; structure and human readability are allies.
...
- [ ] Description matches 3 realistic phrasings you generate now, rejects 1 nearby-but-wrong phrasing, and doesn't collide with any existing description in `.claude/skills/` — if a plausible phrasing matches two commands, tighten one (usually the new one).
...
**At publish time, add:** the `.claude/skills/` path, a suggested first real invocation, how user will recognize it's working in observable terms, and untested paths flagged honestly.
...
2. **Never publish without user's go-ahead.** `05-finals/`, `.claude/skills/`, and `.github/agents/` receive only the approved. Commissioned work is delegated from the project folder, never published.
...
15. **Never work outside your lanes** — the project folder, `.claude/skills/` and `.github/agents/` on publish, and scoped git operations. Nothing else.
