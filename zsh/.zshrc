...
################################################################
# > MARK: General environment variables
################################################################
...
################################################################
# > MARK: Miscellaneous configuration values
################################################################
export HOMEBREW_AUTO_UPDATE_SECS=2592000 # 30 days

################################################################
# > MARK: Only apply to interactive shells
################################################################
[ -z "$PS1" ] && return
...
# >> MARK: fzf - Source key bindings etc
...
################################################################
# > MARK: runscript: safe remote script runner
################################################################
#   Usage:
...
alias vnvtheme='$VS ~/.config/nvim/colors/monokai-fusion.vim'
...
################################################################
#
#   MARK: Functions - Cross-Platform
#
################################################################

################################################################
# > MARK: b62-9 - generate a base62 string (9 characters)
################################################################
function b62-9 {
  local DIGITS=9
...
################################################################
# > MARK: cd improved - list directory contents after cd
################################################################
...
################################################################
# > MARK: dall - delete all silly things
################################################################
dall() {
...
################################################################
# > MARK: key - add identity to funtoo/keychain
################################################################
key() {
...
################################################################
# > MARK: fh - search command history with fzf and execute selected command
################################################################
fh() {
...
################################################################
# > MARK: gcp - git cherry-pick helper
################################################################
...
################################################################
# > MARK: minipng - compress all pngs in current directory (pngquant)
################################################################
...
################################################################
# > MARK: renumdir - Renumber all files in directory
################################################################
...
################################################################
# > MARK: snap* - archives files and dirs
################################################################
snapc() {
  if [[ $# -ne 1 ]]; then
...
if [[ "$(uname)" == "Darwin" ]]; then
  ...
  ################################################################
  # > MARK: cpdl - copy latest download
  ################################################################
  cpdl() {
  ...
  ################################################################
  # > MARK: cpshot - copy latest screenshot
  ################################################################
  cpshot() {
  ...
  ################################################################
  # > MARK: list_xcode_provisioning_profiles
  ################################################################
  ...
  ################################################################
  # > MARK: tesso, tessoshot - tesseract shortcuts - ocr (Mac)
  ################################################################
...
################################################################
#
#   MARK: Functions - for Windows (WSL Ubuntu)
#
################################################################
...
if command -v wslpath &> /dev/null; then
  ...
  ################################################################
  # > MARK: open - mimics Mac open in Ubuntu WSL
  ################################################################
  function open {
  ...
  ################################################################
  # > MARK: tesso, tessoshot - tesseract shortcuts - ocr (Win)
  ################################################################
  if command -v tesseract &> /dev/null; then
    tesso() {
      tesseract "$1" stdout
  ...
  ################################################################
  # > MARK: winvar - echo value of Windows environment variable
  ################################################################
  winvar() {
...
################################################################
#
#   MARK: AWS Helper Functions
#
################################################################

################################################################
# > MARK: rdsconnect - connect to postgresql RDS instance via ec2 target at fixed port
################################################################
rdsconnect() {
  if [[ -z "$1" ]]; then
...
################################################################
# > MARK: s3hash - generate a s3 friendly hash string (25 chars)
################################################################
function s3hash {
...
################################################################
# > MARK: s3hashalljpg - rename jpgs in dir with hash, and preserve originals
################################################################
...
################################################################
# > MARK: s3hashcp - rename a copy of the given file with a s3 friendly hash name
################################################################
function s3hashcp {
  # Require only one argument
  if [[ $# -ne 1 ]]; then
...
################################################################
# > MARK: s3hashrename - rename the given file with a s3 friendly hash name
################################################################
